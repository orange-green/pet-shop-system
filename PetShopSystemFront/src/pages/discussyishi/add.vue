...
<div :style='{"width":"80%","padding":"20px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.3)","margin":"10px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="80px"
    >
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}' label="头像" v-if="type!='cross' || (type=='cross' && !ro.avatarurl)" prop="avatarurl">
            <file-upload
            tip="点击上传头像"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}' class="upload" v-else label="头像" prop="avatarurl">
          ...
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}' label="用户名" prop="nickname">
            <el-input v-model="ruleForm.nickname"
                placeholder="用户名" clearable :disabled=" false  ||ro.nickname"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}' label="评论内容" prop="content">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="评论内容"
          ...
          <el-form-item :style='{"width":"50%","padding":"10px","margin":"0 0 10px","background":"none"}' label="回复内容" prop="reply">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="回复内容"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`discussyishi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('discussyishi/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`discussyishi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
