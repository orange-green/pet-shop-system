...
<div>
	<div :style='{"border":"1px solid #E0E0E0","padding":"20px 0 10px","margin":"10px auto","borderRadius":"0","background":"none","borderWidth":"0 0 2px","display":"flex","width":"80%","justifyContent":"center"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/discussyishi"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"minHeight":"560px","padding":"0 20px","margin":"0 0 10px","background":"#fff","flex":"1","display":"flex","width":"55%","position":"relative","order":"2"}'>
			...
			<div class="info" :style='{"padding":"0 10px","margin":"0 0 0 10px","background":"#fff","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"0px","margin":"0 0 0 0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#818181","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>关联表id</div>
					<div  :style='{"padding":"9px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#818181","flex":"1","height":"auto"}'>{{detail.refid}}</div>
				</div>
				<div class="item" :style='{"padding":"0px","margin":"0 0 0 0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					...
					<div  :style='{"padding":"9px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#818181","flex":"1","height":"auto"}'>{{detail.nickname}}</div>
				</div>
				<div class="item" :style='{"padding":"0px","margin":"0 0 0 0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#818181","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>评论内容</div>
					<div  :style='{"padding":"9px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#818181","flex":"1","height":"auto"}'>{{detail.content}}</div>
				</div>
				<div class="item" :style='{"padding":"0px","margin":"0 0 0 0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#818181","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>回复内容</div>
					<div  :style='{"padding":"9px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#818181","flex":"1","height":"auto"}'>{{detail.reply}}</div>
				</div>
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","justifyContent":"space-between","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 5px 10px 0","outline":"none","color":"#fff","borderRadius":"20px","background":"#3B2E7E","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('discussyishi','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"1px solid #CCCCCC","cursor":"pointer","padding":"0 20px","margin":"0 5px 10px 0","outline":"none","color":"#CCCCCC","borderRadius":"20px","background":"none","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('discussyishi','删除')" @click="delClick">删除</el-button>
					<el-button :style='{"border":"1px solid #CCCCCC","cursor":"pointer","padding":"0 20px","margin":"0 5px 10px 0","outline":"none","color":"#CCCCCC","borderRadius":"20px","background":"none","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('discussyishi','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
				</div>
			</div>
		</div>
    ...
    methods: {
      ...
      nextClick(page) {
		...
		backClick(){
			...
			this.$router.push({path: '/index/discussyishi', query: params});
		},
		// 下载
		download(file){
			if(!file) {
      ...
      getDiscussList(page) {
        this.$http.get('discussdiscussyishi/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('discussdiscussyishi/delete',[id]).then(res=>{
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussdiscussyishi/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
	  ...
	  addDiscussNum(type){
		  ...
		  this.$http.post('discussyishi/update',this.detail).then(res=>{
		...
		// 修改
		editClick(){
			this.$router.push(`/index/discussyishiAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此医师评论表？')
			  .then(_ => {
			    this.$http.post('discussyishi/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
						this.$message({
							type: 'success',
							message: '删除成功!',
