...
<div>
	<div :style='{"border":"1px solid #E0E0E0","padding":"20px 0 10px","margin":"10px auto","borderRadius":"0","background":"none","borderWidth":"0 0 2px","display":"flex","width":"80%","justifyContent":"center"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"14px","lineHeight":"1"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/yishi"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="attr" :style='{"minHeight":"560px","padding":"0 20px","margin":"0 0 10px","background":"#fff","flex":"1","display":"flex","width":"55%","position":"relative","order":"2"}'>
			...
			<div class="info" :style='{"padding":"0 10px","margin":"0 0 0 10px","background":"#fff","flex":"1"}'>
				...
				<div class="item" :style='{"padding":"0px","margin":"0 0 0 0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					...
					<div  :style='{"padding":"9px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#818181","flex":"1","height":"auto"}'>{{detail.yishixingming}}</div>
				</div>
				<div class="item" :style='{"padding":"0px","margin":"0 0 0 0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
				...
				<div class="item" :style='{"padding":"0px","margin":"0 0 0 0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#818181","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>职位</div>
					<div  :style='{"padding":"9px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#818181","flex":"1","height":"auto"}'>{{detail.zhiwei}}</div>
				</div>
				<div class="item" :style='{"padding":"0px","margin":"0 0 0 0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#818181","textAlign":"right","width":"102px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>电话</div>
					<div  :style='{"padding":"9px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#818181","flex":"1","height":"auto"}'>{{detail.dianhua}}</div>
				</div>
				<div class="item" :style='{"padding":"0px","margin":"0 0 0 0","background":"#fff","justifyContent":"spaceBetween","display":"flex"}'>
				...
				<div class="btn" :style='{"padding":"10px 0","flexWrap":"wrap","justifyContent":"space-between","display":"flex"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 5px 10px 0","outline":"none","color":"#fff","borderRadius":"20px","background":"#3B2E7E","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('yishi','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"1px solid #CCCCCC","cursor":"pointer","padding":"0 20px","margin":"0 5px 10px 0","outline":"none","color":"#CCCCCC","borderRadius":"20px","background":"none","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('yishi','删除')" @click="delClick">删除</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 5px 10px 0","outline":"none","color":"#fff","borderRadius":"20px","background":"#6353B4","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('yishi','问诊')" @click="onAcross('zaixianwenzhen','','','','')" type="warning">问诊</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","margin":"0 5px 10px 0","outline":"none","color":"#fff","borderRadius":"20px","background":"#6353B4","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('yishi','预约')" @click="onAcross('zaixianyuyue','','','','')" type="warning">预约</el-button>
				</div>
			</div>
		</div>
  ...
  export default {
    ...
    methods: {
      ...
      async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
        localStorage.setItem('crossTable',`yishi`);
      ...
      storeup(type) {
        if (type == 1 && !this.isStoreup) {
          ...
          this.$http.post('storeup/add', this.storeupParams).then(res => {
            if (res.data.code == 0) {
			  ...
			  this.$http.post('yishi/update', this.detail).then(res => {});
              this.$message({
                type: 'success',
                message: '收藏成功!',
                duration: 1500,
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'yishi', userid: localStorage.getItem('frontUserid')}}).then(res => {
      ...
      nextClick(page) {
		...
		backClick(){
			...
			this.$router.push({path: '/index/yishi', query: params});
		},
		// 下载
		download(file){
			if(!file) {
      ...
      getDiscussList(page) {
        this.$http.get('discussyishi/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
	  ...
	  discussDel(id){
		  this.$confirm('是否删除此评论？')
		    .then(_ => {
		      this.$http.post('discussyishi/delete',[id]).then(res=>{
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussyishi/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
	  ...
	  addDiscussNum(type){
		...
		// 修改
		editClick(){
			this.$router.push(`/index/yishiAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此医师？')
			  .then(_ => {
			    this.$http.post('yishi/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
